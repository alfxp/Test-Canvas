var UnoDropZone = {
    settings: {
        ...
        defaultMessage: 'Clique aqui ou arraste a imagem aqui'
    },
    util: {
    ...
    handleFiles: function (fileList, inputElemnt) {
        ...
        $("#SizeO" + inputElemnt.name).html(" " + UnoDropZone.util.bytesToSize(file.size));
        
        // check type 
        var imageType = /^image\//;
        if (!imageType.test(file.type)) {
            console.log('Não consigo reconhecer esse arquivo como uma imagem válida.');
            $(this).val('');
            return;
        }
        ...
        filethumbnailImg.setAttribute("name", "img");
        filethumbnailImg.setAttribute("Id", "img" + inputElemnt.name);        
